...
class BuyOptimizer {
    ...
    fun <T> solve(problem: VendorProblem<T>): VendorSolution<T> {
        val solver = MPSolver("SolveIntegerProblem", MPSolver.OptimizationProblemType.CBC_MIXED_INTEGER_PROGRAMMING)

        val variablesForOptions = problem.purchaseOptions.map { option ->
            val variable = solver.makeIntVar(0.0, option.availableQuantity.toDouble(), "${option.key}_quantity")
        ...
        // of the problem.
        val buyFlagsByVendor = variablesByVendor.keys.associate { vendorName ->
            vendorName to solver.makeIntVar(0.0, 1.0, "buyFlag_$vendorName")
        }

        // We need the desired amount of each good.
        problem.goodQuantitiesSought.forEach { (good, quantity) ->
            val desiredAmount = solver.makeConstraint(quantity.toDouble(), quantity.toDouble())
            val variables = variablesByGood[good] ?: throw UnsolvableException("Desire $good but it is unavailable")
            variables.forEach { desiredAmount.setCoefficient(it.variable, 1.0) }
        }
        ...
        variablesByVendor.forEach { (vendorName, variables) ->
            val buyAnything = solver.makeConstraint(Double.NEGATIVE_INFINITY, 0.0)
            buyAnything.setCoefficient(buyFlagsByVendor[vendorName], -LARGE_NUMBER)
            variables.forEach { buyAnything.setCoefficient(it.variable, 1.0) }
        }
        ...
        buyFlagsByVendor.forEach { (vendorName, buyFlag) ->
            val minimumSpend = solver.makeConstraint(Double.NEGATIVE_INFINITY, 0.0)
            minimumSpend.setCoefficient(buyFlag, problem.minimumRequiredPurchase.toDouble())
            variablesByVendor[vendorName]?.forEach {
                minimumSpend.setCoefficient(it.variable, -it.option.price.toDouble())
            }
        }

        val totalCost = solver.objective()
        variablesForOptions.forEach {
            totalCost.setCoefficient(it.variable, it.option.price.toDouble())
        }
        totalCost.setMinimization()

        val resultStatus = solver.solve()
        if (resultStatus != MPSolver.ResultStatus.OPTIMAL) {
            throw UnsolvableException("Problem was not mathematically solvable; returned status $resultStatus")
        }
